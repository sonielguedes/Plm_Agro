...
class WialonIpsClient {
    private val TAG = "WIALON_IPS_TCP"
    ...
    private fun ensureConnected(host: String, port: Int, imei: String) {
        if (socket == null || socket?.isClosed == true || socket?.isConnected == false) {
            ...
            Log.i(TAG, "IPS_TCP_CONNECTING: $host:$port")
            ...
            try {
                ...
                Log.i(TAG, "IPS_TCP_LOGIN_SENT: $imei")
                ...
                Log.i(TAG, "IPS_TCP_LOGIN_ACK: $response")
        ...
        mutex.withLock {
            ...
            try {
                ...
                messages.forEach { msg ->
                    ...
                    Log.i(TAG, "IPS_TCP_PACKET_SENT: ${msg.trim()}")
                    ...
                    if (bytesRead > 0) {
                        ...
                        if (resp.contains("#AD#1")) {
                            Log.i(TAG, "IPS_TCP_PACKET_ACK: $resp (${latency}ms)")
                            lastAck = resp
                        } else if (resp.contains("#AD#-1")) {
                            Log.e(TAG, "IPS_TCP_PACKET_REJECTED: Erro de dados (#AD#-1)")
                            throw Exception("Protocolo: Dados Rejeitados (#AD#-1)")
                        } else {
                            Log.w(TAG, "IPS_TCP_UNEXPECTED_RESP: $resp")
