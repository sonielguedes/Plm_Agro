...
### 3.2. Sincronização de Motoristas (Login)
*   **Problema:** A tela de login não exibia todos os motoristas cadastrados.
*   **Solução:**
...
### 3.3. Recuperação de Dados Avançados da Unidade
*   **Problema:** Ao vincular um equipamento, campos como **Placa**, **Tipo** e **KM** ficavam vazios.
*   **Solução:** Atualizada a função `getUnitData` no `WialonRepository.kt` para solicitar flags de propriedades personalizadas e contadores (`0x1 | 0x100 | 0x400`). O sistema agora preenche automaticamente a Placa e o KM inicial com base nos dados do servidor.
